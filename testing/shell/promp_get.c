...
int main(void)
{
    ...
    char *delimiters = " ";
    char **tokens = malloc(128); /* Allocate memory for tokens */

    /* The Prompt */
    write(1, "-$ ",3);

    /*Get user input */
     chars_read = getline(&line, &lenght, stdin);
     if (chars_read == -1)
     {
     ...
     else
     {
        ...
        while( tokens != NULL)
        {
           ...
           else if (pid == 0)
           {
            ...
            execve(cmd_path, &tokens, __environ);
            if (execve(cmd_path, &tokens, __environ) == -1)
            {
